...
get_inactive_color() {
   ...
   # while IFS="," read -r line; do fadeincolorRGB+=("$line"); done < <(kreadconfig5 --key "Color" --group "ColorEffects:Inactive" --file "$KDE_GLOBALS_FILE");
    ...
    colorRGB[0]=$(round_to_int "${colorRGB[0]}");
    colorRGB[1]=$(round_to_int "${colorRGB[1]}");
    colorRGB[2]=$(round_to_int "${colorRGB[2]}");
...
COLOR_TO_USE="#$(rgb2hex "$(get_inactive_color "$COLOR_TO_USE")")"
PATTERN="{{$3}}";
